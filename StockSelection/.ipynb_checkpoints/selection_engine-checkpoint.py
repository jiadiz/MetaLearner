...
def run_rolling_backtest_selection(
    ...
    *,
    ...
    eval_dates = pd.Series(unique_dates[-(cfg.horizon_bd * lookback_windows + 2) :]).iloc[::-spacing_days].to_list()

    rows = []
    for dt in eval_dates:
        rows.append(run_experiment_single_day(frame, price_df, dt, config=cfg))
